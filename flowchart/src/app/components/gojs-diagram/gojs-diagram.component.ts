...
import { Intent_Task } from 'src/Models/Intent_Task';
import { ScreenAutomator_Task } from 'src/Models/ScreenAutomator_Task';
...
export class GojsDiagramComponent implements AfterViewInit {
  ...
  initDiagram(): void {
    ...
    this.myDiagram.nodeTemplateMap.add(
      ...
      new go.Node('Auto').apply(nodeStyle).add(
        ...
        new go.TextBlock({
          ...
          .apply(textStyle)
          .bindTwoWay('text'),
        NodeImage('assets/css/accessibilite.png')
      )
    );
    this.myDiagram.nodeTemplateMap.add(
      'Intent',
      new go.Node('Auto').apply(nodeStyle).add(
        ...
        new go.TextBlock({
          ...
          .apply(textStyle)
          .bindTwoWay('text'),
        NodeImage('assets/css/fleche-droite.png')
      )
    );
    ...
    this.myPalette.nodeTemplateMap.add(
      'Screen Automator', // the default category , '' :
      new go.Node('Auto')
        .apply(nodeStyle) //Crée un nœud avec un layout automatique ('Auto'), ce qui signifie que son contenu sera ajusté à sa forme.
        .add(
          new go.Shape('RoundedRectangle', {
          ...
          new go.TextBlock({
            ...
            .bindTwoWay('text'),
          new go.Picture({
            source: 'assets/css/accessibilite.png', // Correction du chemin d'image
            desiredSize: new go.Size(20, 20),
            alignment: new go.Spot(0.05, 0.5), // Positionnement
          })
        )
      ...
      new go.Node('Auto')
        ...
        .add(
          ...
          new go.TextBlock({
            ...
            .bindTwoWay('text'),
          new go.Picture({
            source: 'assets/css/fleche-droite.png', // Correction du chemin d'image
            desiredSize: new go.Size(20, 20),
            alignment: new go.Spot(0.05, 0.5), // Positionnement
          })
        )
    ...
    this.myPalette = new go.Palette('myPaletteDiv', {
      ...
      model: new go.GraphLinksModel([
        ...
        { category: 'Start', text: 'Start', As_thread:false,Time_out:0,Sequential_exec:false},
        ...
        {
          category: 'Screen Automator',
          text: 'Screen Automator',
          description:
            'Screen Automator',
          Action_type:'',
          GlobalAction_type:null,
  ...
  start(): void {
    ...
    let screenAutomatorTasks: ScreenAutomator_Task[] = [];
    let startTasks: Start_Task[] = [];
    let endTasks: End_Task[] = [];
    let intentTasks: Intent_Task[] = []; // <-- Nouveau tableau pour Intent
    ...
    function parseCompositeTask(node: any): Composite_Task {
      ...
      let subScreenAutomatorTasks: ScreenAutomator_Task[] = [];
      let subStartTasks: Start_Task[] = [];
      let subEndTasks: End_Task[] = [];
      let subIntentTasks: Intent_Task[] = [];
      ...
      if (node.pipeline) {
        node.pipeline.nodeDataArray.forEach((subNode: any) => {
          switch (subNode.category) {
            case 'Cmd Stage':
              subCmdStages.push(removeEmptyFields(new CmdStage(
                subNode.key.toString(), subNode.text, subNode.cmd || "", subNode.root ? 1 : 0
              )));
              break;
            case 'Sleep':
              subSleepTasks.push(removeEmptyFields(new Sleep_Task(
                subNode.key.toString(), subNode.text, subNode.Time_sleep
              )));
              break;
            case 'Screen Automator':
              subScreenAutomatorTasks.push(removeEmptyFields(new ScreenAutomator_Task(
                subNode.key.toString(), subNode.text,
                subNode.Action_type, subNode.GlobalAction_type,
                subNode.ClickInText_textSelector, subNode.ClickInText_CompareType,
                +subNode.ClickInText_Index, subNode.ClickInText_text,
                +subNode.ClickInXY_X, +subNode.ClickInXY_Y, +subNode.LogScreen
              )));
              break;
            case 'Start':
              subStartTasks.push(removeEmptyFields(new Start_Task(subNode.key.toString(), subNode.text, subNode.As_thread,subNode.Time_out,subNode.Sequential_exec)));
              break;
            case 'End':
              subEndTasks.push(removeEmptyFields(new End_Task(subNode.key.toString(), subNode.text)));
              break;
            case 'Intent':
              subIntentTasks.push(removeEmptyFields(new Intent_Task(
                subNode.key.toString(), subNode.text,
                subNode.Action, subNode.PackageName, subNode.ClassName,
                subNode.Data, subNode.ActionType
              )));
              break;
            case 'CompositeTask':
      ...
      if (subScreenAutomatorTasks.length) compositeTask.screenAutomatorTasks = subScreenAutomatorTasks;
      if (subStartTasks.length) compositeTask.startTasks = subStartTasks;
      if (subEndTasks.length) compositeTask.endTasks = subEndTasks;
      if (subIntentTasks.length) compositeTask.intentTasks = subIntentTasks;
    ...
    nodeArray.forEach((node: any) => {
      switch (node.category) {
        case 'Cmd Stage':
          cmdStages.push(removeEmptyFields(new CmdStage(
            node.key.toString(), node.text, node.cmd || "", node.root
          )));
          break;
        case 'Sleep':
          sleepTasks.push(removeEmptyFields(new Sleep_Task(
            node.key.toString(), node.text, node.Time_sleep
          )));
          break;
        case 'Screen Automator':
          screenAutomatorTasks.push(removeEmptyFields(new ScreenAutomator_Task(
            node.key.toString(), node.text,
            node.Action_type, node.GlobalAction_type,
            node.ClickInText_textSelector, node.ClickInText_CompareType,
            node.ClickInText_Index, node.ClickInText_text,
            node.ClickInXY_X, node.ClickInXY_Y, node.LogScreen
          )));
          break;
        case 'Start':
          startTasks.push(removeEmptyFields(new Start_Task(node.key.toString(), node.text, node.As_thread,node.Time_out,node.Sequential_exec)));
          break;
        case 'End':
          endTasks.push(removeEmptyFields(new End_Task(node.key.toString(), node.text)));
          break;
        case 'Intent':
          intentTasks.push(removeEmptyFields(new Intent_Task(
            node.key.toString(), node.text,
            node.Action, node.PackageName, node.ClassName,
            node.Data, node.ActionType
          )));
          break;
        case 'CompositeTask':
    ...
    if (screenAutomatorTasks.length) formattedJson['Screen Automator'] = screenAutomatorTasks;
    if (startTasks.length) formattedJson['Start'] = startTasks;
    if (endTasks.length) formattedJson['End'] = endTasks;
    if (intentTasks.length) formattedJson['Intent'] = intentTasks;
    if (compositeTasks.length) formattedJson['CompositeTask'] = compositeTasks;
    if (links.length) formattedJson['Links'] = links;
  
    const outputTextArea = document.getElementById('outputJson') as HTMLTextAreaElement;
    if (outputTextArea) {
